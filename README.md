...
永久删除只能从精确查询结果发起：输入一次完整 Case ID 并搜索，核对结果后点击删除，
勾选“我确认删除这个案件，且无法恢复”，再执行最终确认，无需第二次手输 ID。
现有安全元数据不提供标题或关联记录数量；取消、重新搜索和退出登录会清除旧确认。
删除在一个 PostgreSQL transaction 中删除唯一父 Case，依赖现有
