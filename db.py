...
ARTIFACT_KINDS = PUBLIC_ARTIFACT_KINDS | CHECKPOINT_ARTIFACT_KINDS
    ...
    BEGIN
        IF NOT EXISTS (
            ...
            WHERE conrelid = 'artifacts'::regclass
              ...
              AND pg_get_constraintdef(oid) LIKE '%META_JUDGMENT%'
        ) THEN
...
class Database:
    ...
    def get_case(self, case_id):
        return self._read_query(
            """
            SELECT case_id, title, status, paused_by, created_at, updated_at
            FROM cases
            WHERE case_id = %s
            """,
            (case_id,),
    ...
    def get_artifact(self, case_id, kind):
        ...
        return self._read_query(
            """
            SELECT id, case_id, kind, content, created_at
            FROM artifacts
    ...
    def save_checkpoint(self, case_id, kind, content):
    ...
    def claim_artifact(self, case_id, kind):
        if kind == "DISPUTE_MAP":
            ...
            statement = """
                ...
                FROM cases AS c
                WHERE c.case_id = %s
                  AND c.status IN ('MAP_READY', 'MEDIATING', 'PAUSED')
                ON CONFLICT (case_id, kind) DO NOTHING
                RETURNING id
            """
        else:
    ...
    def add_message(self, case_id, sender, content):
        ...
        with self._connection() as connection:
            ...
            if not case or case["status"] not in {"MAP_READY", "MEDIATING"}:
            ...
            connection.execute(
                """
                UPDATE cases
                SET status = 'MEDIATING', updated_at = NOW()
                WHERE case_id = %s
                """,
                (case_id,),
            )
